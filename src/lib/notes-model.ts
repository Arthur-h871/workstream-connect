...
export function ensureAnchorLine(
  lines: TextLine[],
  lineId: string,
  todoBlockId: string,
): { lines: TextLine[]; lineId: string } {
  ...
  if (
    targetLine.anchorTodoBlockId === todoBlockId ||
    (!targetLine.anchorTodoBlockId && targetLine.text.trim().length === 0)
  ) {
    return {
      lineId: targetLine.id,
      lines: attachTodoToLine(lines, targetLine.id, todoBlockId),
    };
  }
  ...
  const nextLine: TextLine = {
    id: crypto.randomUUID(),
    text: "",
    anchorTodoBlockId: todoBlockId,
  };

  return {
    lineId: nextLine.id,
    lines: [...lines.slice(0, targetIndex + 1), nextLine, ...lines.slice(targetIndex + 1)],
  };
}

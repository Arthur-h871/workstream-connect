...
test("notes breadcrumb is centered at the top instead of overlapping the left toolbar", () => {
  const notesCanvasPage = readProjectFile("./src/components/notes/NotesCanvasPage.tsx");

  assert.match(
    notesCanvasPage,
    /className="pointer-events-auto absolute left-1\/2 top-4 z-20 flex flex-wrap items-center gap-1 rounded-2xl border border-border bg-surface\/95 px-3 py-2 text-xs shadow-lg backdrop-blur -translate-x-1\/2"/,
  );
  assert.doesNotMatch(notesCanvasPage, /left-4 top-24/);
});
...
test("notes canvas page centers new notes using the immersive canvas viewport bounds", () => {
  ...
  assert.match(
    notesCanvasPage,
    /const viewportCenter = useCallback\(\(\) => \{[\s\S]*const viewportRect = viewportRef\.current\?\.getBoundingClientRect\(\);[\s\S]*return screenToFlowPosition\(\{[\s\S]*x:\s*\(viewportRect\?\.left \?\? 0\) \+ \(viewportRect\?\.width \?\? window\.innerWidth\) \/ 2,[\s\S]*y:\s*\(viewportRect\?\.top \?\? 0\) \+ \(viewportRect\?\.height \?\? window\.innerHeight\) \/ 2,[\s\S]*\}\);[\s\S]*\}, \[screenToFlowPosition\]\);/,
  );
...
test("todo list node expands with content instead of using internal scroll", () => {
  const todoListNode = readProjectFile("./src/components/canvas/TodoListNode.tsx");

  assert.match(todoListNode, /body\.scrollHeight/);
  assert.match(todoListNode, /typedData\.onResizeEnd\(id, root\.getBoundingClientRect\(\)\.width, targetHeight\)/);
  assert.doesNotMatch(todoListNode, /overflow-auto/);
});

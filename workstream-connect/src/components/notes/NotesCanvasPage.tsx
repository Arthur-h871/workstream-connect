...
function InnerCanvas() {
  ...
  const syncAnchoredTodoNodes = useCallback((inputNodes: Node[]) => {
    ...
    const anchoredTodosByTextBlock = new Map<string, AnchoredTodoView[]>();

    for (const node of inputNodes) {
      if (node.type !== "todo") continue;
      const content = (node.data as { content: TodoBlockContent }).content;
      if (!isTodoAnchored(content) || !content.anchor.textBlockId || !content.anchor.lineId) continue;

      const existing = anchoredTodosByTextBlock.get(content.anchor.textBlockId) ?? [];
      existing.push({
        blockId: node.id,
        lineId: content.anchor.lineId,
        content,
      });
      anchoredTodosByTextBlock.set(content.anchor.textBlockId, existing);
    }

    return inputNodes.map((node) => {
      if (node.type === "text") {
        const anchoredTodos = (anchoredTodosByTextBlock.get(node.id) ?? []).sort((left, right) => {
          const content = (node.data as { content: TextBlockContent }).content;
          const leftIndex = content.lines.findIndex((line) => line.id === left.lineId);
          const rightIndex = content.lines.findIndex((line) => line.id === right.lineId);
    ...
    (block: NoteBlock): Node => {
      ...
      return {
        ...
        data: {
          ...
          onTodoContentChange: handleUpdateTodoContent,
          onTodoTaskStatusChange: handleTodoTaskStatus,
          onRequestUnanchorTodo: handleRequestUnanchorTodo,
          onOpenPopup: handleOpenPopup,
          onResizeEnd: handleResizeEnd,
        },
        style: { width: block.width, height: block.height },
  ...
  const maybeAnchorTodoToText = useCallback(
    async (todoNode: Node) => {
      ...
      const anchorResult = ensureAnchorLine(textContent.lines, line.id, todoNode.id);
      const nextTextContent: TextBlockContent = {
        ...textContent,
        text: anchorResult.lines.map((entry) => entry.text).join("\n"),
        lines: anchorResult.lines,
      };
      const todoContent = (todoNode.data as { content: TodoBlockContent }).content;
      const nextTodoContent: TodoBlockContent = {
        ...todoContent,
        anchor: { textBlockId: droppedTextNode.id, lineId: anchorResult.lineId },
      };
  ...
  const breadcrumbView = useMemo(
    () => (
      <div className="pointer-events-auto absolute left-1/2 top-4 z-20 flex flex-wrap items-center gap-1 rounded-2xl border border-border bg-surface/95 px-3 py-2 text-xs shadow-lg backdrop-blur -translate-x-1/2">

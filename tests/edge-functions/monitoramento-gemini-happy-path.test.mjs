...
test("monitoramento start/chat happy path with ai_provider: gemini", { skip }, async (t) => {
  const admin = createClient(SUPABASE_URL, SERVICE_ROLE_KEY, {
    auth: { autoRefreshToken: false, persistSession: false },
  });
  const anon = createClient(SUPABASE_URL, ANON_KEY);

  const suffix = `gemini-${process.pid}-${t.name.length}-${Object.keys(process.env).length}`;
  ...
  const { data: org, error: orgError } = await admin
    .from("organizations")
    .insert({ name: `Fixture Org ${suffix}`, ai_provider: "gemini" })
    .select("id")
    .single();
